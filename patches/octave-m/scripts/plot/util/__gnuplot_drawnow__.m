...
function __gnuplot_drawnow__ (h, term, file, debug_file)
  ...
  if (nargin >= 3 && nargin <= 4)
    ...
    ## WEBASSEMBLY OVERRIDE: the stock code reuses the previous stream when
    ## this figure has been drawn before (get "__plot_stream__"), which makes
    ## the single/stream file append forever.  In the web build each figure
    ## draws to its own file (/plot-fig-<handle>.gp, see
    ## __gnuplot_open_stream__) and that file is truncated on every draw, so
    ## always opening a fresh stream keeps every figure's file a clean single
    ## block between JS renders.
    plot_stream = __gnuplot_open_stream__ (2, h);
    new_stream = true;
