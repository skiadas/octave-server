...
Status: **M0–M3 done — app shell refactored to ES modules + esbuild build (dist + single-file profiles); per-figure plot pipeline live: each figure renders to its own `/plot-fig-*.gp` stream (truncated on redraw), JS renders every changed figure into run-grouped gallery entries with a one-SVG viewer (◀ i / N ▶); file panel hardened with an icon toolbar + clickable breadcrumb target bar + up-to-parent and now renders the persisted tree immediately (no blank panel until Octave boots); workspace layout is collapsible — the 240px file panel is **collapsed by default** (header `Files` toggle or Ctrl/Cmd+B, state persisted in `localStorage` via `app/layout.js` + a head inline no-flash script), and drag-to-upload works across the whole workspace (`#workspace`) even while the panel is hidden; the editor is a highlighted overlay now — `#editor` stays the real `<textarea>` (the value source of truth) with a syntax-highlight mirror + line gutter behind it (highlight.js + the highlightjs-octave grammar, vendored + bundled, offline), a Ctrl+Space completion popup fed from a curated ~230-name Octave list, and a localStorage draft of `{text, filename}` so a reload doesn't wipe the buffer (`app/editorpad.js`); cache-busting live: `dist/app/index.html` generated with `?v=<hash>` asset URLs + `window.__OO_V__` consumed by `main.js` `locateFile`, and `scripts/serve.py` sends `Cache-Control: no-store` so stale wasm/data can never replay; UI-only iterations are now a fast no-boot loop — `ui-unit` 65/65, `index-check` 9/9, `ui-dom` 15/15 — wired into a dedicated UI CI gate (no Docker) while the wasm gate (`verify.yml`) and the Deploy wasm-artifact cache make app-only pushes skip the ~2–15 min Docker rebuild; render battery `verify:fast` 10/10 green; smoke 5/5, single-file 3/3 green; Octave Forge `statistics` 1.6.0 + `data-smoothing` 1.3.0 baked in; symbolic (SymPy) shim green.**
...
- **UI unit** `node ui-unit.mjs`: **65/65 pass** — Phase D breadcrumb/navigation
  ...
  clear-all) and the `locateFile` `?v=`/data-URI priority rules, Phase F
  layout (default-collapsed file panel, header toggle flips + persists,
  Ctrl+B recolapses, whole-workspace drop uploads while collapsed), and
  Phase G editor overlay (mirror renders token classes, gutter line counts,
  Ctrl+Space opens the completion popup with `plot` on top, Enter accepts,
  ArrowDown/Escape navigate/dismiss, Tab indents or opens completion by
  context, and the draft persists to / restores from `localStorage`).
...
- **UI DOM shell** `node ui-dom.mjs`: **15/15 pass** — real headless Chrome loads
  ...
  shell panels present, editor is a `<textarea>` with the overlay elements
  (gutter/mirror/popup/probe) present, typing into the editor colors the mirror
  (`hljs-comment`) + counts lines in the gutter, Ctrl+Space shows the completion
  popup with candidates, the file-panel bar renders its buttons, viewer controls
  present, status is non-fatal — and the workspace layout is collapsed by
  default, the header `Files` toggle reveals the panel (computed display flips),
  and Ctrl+B recolapses it. The harness servers set `SO_REUSEADDR` so rapid
  local reruns never hit `EADDRINUSE` from a previous run's `TIME_WAIT` socket.
