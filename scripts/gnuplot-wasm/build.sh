...
function get {
  ...
  patch "$BUILD/gnuplot/src/plot.c" < "$ROOT/batch-mode.patch"
}
...
function build {
  ...
  emmake make \
    CFLAGS="-Oz -flto" \
    CXXFLAGS="-Oz -flto" \
    LDFLAGS="--js-transform $ROOT/apply_template.sh --pre-js $ROOT/template/pre.js --closure 1 -sINVOKE_RUN=0 -sENVIRONMENT=web -sINCOMING_MODULE_JS_API=arguments,printErr,onAbort,onRuntimeInitialized,instantiateWasm" \
    EXEEXT=".js" \
    gnuplot
}
